...
export default class Calendar extends Component{
    ...
    constructor(props, context) {
        ...
        this.state = {
            ...
            currentDate:this.props.defaultDate || new Date(),
    ...
    getDates(arr,selectedDate,defaultDate){
        ...
        for(let j=1;j<=7;j++){
            ...
            if(arr.length > 0){
                ...
                } onClick={disabled ? '':_this.dateClick.bind(_this,`${year}/${month+1}/${d}`)} >{d}</span></td> );
            }
        }
        return dom;
    }
    ...
    getYears(){
        ...
        let year = parseInt(selectDate.year / 10, 10) * 10;
            ...
            for (let i = -1; i < 11; i++) {
                list.push(
                    <span><i className={classnames({
                                    [_this.getClassName('selected',false)]:selectDate.year == y,
                                    old:i == -1 || i == 10
                                })}>{y}</i></span>
                );
                y += 1;
        ...
        return (
            ...
            <table className="calendar">
                ...
                <th>
                    <div className={this.getClassName('icon-prev',false)}></div>
                </th>
                <th colSpan="5">
                    <div className="title">{`${year}年-${year + 9}年`}</div>
                </th>
                <th>
                    <div className={this.getClassName('icon-next',false)}></div>
                </th>
                </thead>
                <tbody>
                        ...
                        <td colSpan="7">
                        {
                            this.month.map(function(item){
                                return <span><i className={classnames({
                                    [_this.getClassName('selected',false)]:selectDate.month == item&& selectDate.year == _this.today.getFullYear()
    ...
    switchYear(type){
        ...
        this.setState({
            selectedDate:new Date(year+ eval("(" + type + ")"),selected.getMonth(),1 )
        });
    }
    ...
    switchMonth(type){
        ...
        this.setState({
            selectedDate:new Date(year,isNaN(type)?month + eval("(" + type + ")"):type,1 )
        });
    }
